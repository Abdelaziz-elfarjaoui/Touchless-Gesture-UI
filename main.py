...
def read_arduino_state():
    ...
    try:
        ...
        while ser.in_waiting > 0:
            ...
            # MUSIC
            elif line.startswith("STATE:MUSIC:"):
                music_state = (line.split(":")[-1] == "PLAYING")
...
def draw_menu(
    ...
    items = [
        ("LED MANAGEMENT", 95, 140),
        ("MOTOR SPEED MANAGEMENT", 150, 195),
        ("MUSIC", 205, 250),
        ("SETTINGS", 260, 305),
        ("QUIT", 315, 360)
    ]
    ...
    cv2.putText(
        frame,
        "● ONLINE" if ser is not None else "● OFFLINE",
        (455, 60),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        GREEN if ser is not None else RED,
        1,
        cv2.LINE_AA
    )
...
def draw_music(frame, fingertip=None):

    draw_ui_frame(frame, "MUSIC")

    cv2.putText(
        frame,
        "STATUS : PLAYING" if music_state else "STATUS : STOPPED",
        (190, 105),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.65,
        GREEN if music_state else WHITE,
        2,
        cv2.LINE_AA
    )
    ...
    controls = [
        ("PLAY", 70, 150, 275, 205),
        ("STOP", 365, 150, 570, 205),
        ("NEXT", 70, 225, 275, 280),
        ("BACK", 365, 225, 570, 280)
    ]

    for text, x1, y1, x2, y2 in controls:
        active = (
            fingertip is not None
            and x1 <= fingertip[0] <= x2
            and y1 <= fingertip[1] <= y2
        )
        draw_button(frame, x1, y1, x2, y2, text, active)


def get_music_item(x, y):

    if 70 <= x <= 275 and 150 <= y <= 205:
        return "PLAY"
    if 365 <= x <= 570 and 150 <= y <= 205:
        return "STOP_MUSIC"
    if 70 <= x <= 275 and 225 <= y <= 280:
        return "NEXT"
    if 365 <= x <= 570 and 225 <= y <= 280:
        return "BACK"
...
def draw_settings(frame, fingertip=None):

    draw_ui_frame(frame, "SETTINGS")

    controls = [
        ("SYSTEM STATUS", 70, 125, 570, 180),
        ("BACK", 70, 225, 570, 280)
    ]
...
def get_settings_item(x, y):

    if 70 <= x <= 570 and 125 <= y <= 180:
        return "STATUS"
    if 70 <= x <= 570 and 225 <= y <= 280:
        return "BACK"
...
def draw_status(
    ...
    cv2.putText(frame, f"MUSIC  : {'PLAYING' if music_state else 'STOPPED'}", (70, 220),
                cv2.FONT_HERSHEY_SIMPLEX, 0.58,
                GREEN if music_state else RED, 1, cv2.LINE_AA)
...
def process_menu_item(item):
    ...
    if item == "LED":
        ...
        send_command("MUSIC_PAGE")
        log_event("Opened Music")

    elif item == "SETTINGS":

        current_page = PAGE_SETTINGS
        send_command("SETTINGS_PAGE")
        log_event("Opened Settings")
...
def process_music_item(item):
    ...
    if item == "PLAY":
        send_command("PLAY")
        music_state = True
        log_event("Music -> PLAY")

    elif item == "STOP_MUSIC":
        send_command("STOP_MUSIC")
        music_state = False
        log_event("Music -> STOP")

    elif item == "NEXT":
        send_command("NEXT")
        log_event("Music -> NEXT")

    elif item == "BACK":
        current_page = PAGE_MENU
        send_command("BACK")
        log_event("Returned to Main Menu")
...
def process_settings_item(item):

    global current_page

    if item == "STATUS":
        current_page = PAGE_STATUS
        send_command("STATUS_PAGE")
...
# USERNAME / PASSWORD field selection
FIELD_HOVER_TIME = 0.20
    ...
    while True:
        ...
        if current_page == PAGE_LOGIN:
            ...
            if item is None:
                ...
                if item in ("USERNAME", "PASSWORD"):

                    if keyboard_hovered != item:
                        keyboard_hovered = item
                        keyboard_consumed = False
                        keyboard_hover_start = time.time()

                    elif (
                        not keyboard_consumed
                        and time.time() - keyboard_hover_start >= FIELD_HOVER_TIME
                    ):
                        process_login_item(item)
                ...
                # =================================================
                    ...
                    if keyboard_hovered != item:
                        ...
                        if (
                            not keyboard_consumed
                            and time.time() - keyboard_hover_start >= KEY_HOVER_TIME
                        ):
                            process_login_item(item)
                            keyboard_consumed = True
        ...
        elif current_page == PAGE_MUSIC:

            draw_music(frame, fingertip)

            item = None
            if fingertip is not None:
                item = get_music_item(fingertip[0], fingertip[1])
        ...
        # =================================================
            ...
            draw_settings(frame, fingertip)

            item = None
            if fingertip is not None:
                item = get_settings_item(fingertip[0], fingertip[1])
